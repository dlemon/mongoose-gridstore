...
(function() {    
    ...
    var saveToGridStore = function(attachment) {
        ...
        var gridStore = new _mongoose.mongo.GridStore(db, attachment._gsId,'w', {metadata:metadata,content_type:attachment.mimetype});           
    ...
    var readFromGridStore = function(attachment, partial) {
        ...
        var gridStore = new _mongoose.mongo.GridStore(db,attachment._gsId,'r');
        gridStore.open(function(err,gs) {
            if(err) return deferred.reject(err);
            gs.seek(0,function() {
                gs.read(function(err,data) {
                    ...
                    if (loadPartial) {
                        attachment.buffer = new Buffer(0);
                    } else {
                        attachment.buffer = new Buffer(data);
                    }
                    if(gs.metadata) {
    ...
    var removeFromGridStore = function(attachment) {
        ...
        var gridStore = new _mongoose.mongo.GridStore(db,attachment._gsId,'w');
        gridStore.open(function(err,gs) {
            if(err) return deferred.reject(err);           
    ...
    function decorateSchema(mongoose) {                       
        ...
        _schema.methods.addAttachment = function(name, buffer) { 
            ...
            attachment.buffer = new Buffer(buffer);
        ...
        _schema.methods.updateAttachment = function(name, buffer) { 
            ...
            for(var i=0; i<this.attachments.length;i++) {
                if(this.attachments[i].filename == name) {
                    ...
                    this.attachments[i].buffer = new Buffer(buffer);
                }
            }
